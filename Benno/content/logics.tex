...
For a $\sigma$-structure $\str{A}$ and an element $a_{i} \in A$, we write $(\str{A}, a_{i})$ for the $(\sigma \cup {a_{i}})$-structure that extends $\str{A}$ with an additional constant symbol $a_{i}$ interpreted as $a_{i} \in A$, and similar for tuples of elements $\elemtuplea$, where we introduce one constant for each component of the tuple.
...
We commonly visualize structures as directed hypergraphs, as shown in \cref{fig:struct-1}.
...
\begin{figure}[b]
  ...
  The picture shows a structure with a domain of three elements over a signature of four relational symbols. The double border indicates a unary relation, to which only the element $2$ belongs.
  \begin{center}
  \input{res/ex-struct}
  \end{center}
  \textcolor{tolhighcontrastRed}{Red} indicates a binary relation, which contains the tuples $(3,1)$ and $(1,2)$.
  \textcolor{tolhighcontrastBlue}{Blue} indicates a 3-ary relation, which contains the tuple $(1,3,2)$.
  \textcolor{tolhighcontrastYellow}{Yellow} indicates a 4-ary relation, which contains the tuple $(1,1,2,3)$.
  \end{mdframed}
  \caption{An example structure, visualized as directed hypergraph}\label{fig:struct-1}
\end{figure}
...
\begin{multicols}{2}
  \begin{itemize}
    ...
    \item $\forall{x_{i}}(\varphi_{1})$ for $\neg\exists{x_{i}}(\neg \varphi_{1})$
...
\end{definition}
For any modal logic formula $\varphi$, we define $\Box(\varphi) := \neg \Diamond(\neg \varphi)$ and $\varphi \land \psi := \neg ((\neg \varphi) \lor (\neg \psi))$ where $\psi$ is another modal logic formula.
...
\begin{definition}[Forward Guarded Fragment]
  $\FGF(n)$ for $n \in \N$ s the smallest fragment of $\FO$ such that:
  \begin{itemize}\itemsep0em
