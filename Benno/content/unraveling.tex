...
\section{Existing notions of unraveling for $\FGF$ and $\GF$}
...
\begin{example}
  To demonstrate the above definitions, consider the following structure $\str{A}$:\\\vspace{-3em}
  \begin{center}
    \Huge{$\str{A}$}\input{res/ex-other-unravel-struct.tex}
  \end{center}
  ...
  We focus on a substructure of the unraveling here to highlight the interesting cases.
  This substructure is shown in the following picture:
  \begin{center}
  \input{res/ex-other-unravel-gf.tex}
  \end{center}

  Next, we construct the HAH-unraveling $\hatunravel{A}$. First, let us draw the graph of elements which appear consecutively in relations, which gives the following picture:
  \begin{center}
  \input{res/ex-other-unravel-gaifman.tex}
  \end{center}
  The elements of the HAH-unraveling correspond to paths in this graph.
  Since the graph has cycles, the HAH-unraveling is infinite.
  In this example, we focus on the part of the unraveling with paths starting from the element $1$.
  The full unraveling also has paths starting with $2$, $3$, and so on.
  However, since all elements are reachable from $1$ in the above graph, those parts are simply copies of substructures of the unraveling starting from element $1$.
  With this simplification, we get the following (excerpt) of the HAH-unraveling $\hatunravel{A}$:
  \begin{center}
  \input{res/ex-other-unravel-forward.tex}
  \end{center}
\end{example}
...
The new notion of unraveling employs concepts from the $\FGF$-game as introduced in \cref{sec:biseqs-and-bipoints}.
...
At the end of these moves, the selected elements in the game are $\Pi(\sigma)$.
