...
\begin{proof}
  ...
  Let us\bfside{missing word} $\sigma$-structures $\str{A}$ and $\str{B}$.
  ...
  \item $\varphi$ is of the form \bfside{should be $x_{n+1}$ here, since $\varphi(x_{1}, \ldots, x_{n})$ by definition may use $x_{n}$ as free variable. Also, shouldn't this be a single quantifier introducing a tuple instead of many quantifiers introducing one variable at a time?}$\exists{x_{n}} \ldots \exists{x_{m}}\ \relR(x_{i}, \ldots, x_{j}) \land \psi(x_{i}, \ldots, x_{j})$, where $\relR \in \R$ serves as a guard, $i \geq 1$ and $j \leq m$.
  Note that the quantifier rank of $\psi$ is less than $\ell$.\bfside{why highlight the quantifier rank of $\psi$ here, when we use $\heartsuit$ property for $R \land \psi$ below?}
  ...
  \item $\varphi$ is of the form $\exists{x_{1}} \psi(x_1)$. Then the quantifier rank of $\psi$ is $0$\bfside{this is not true, the quantifier rank may be nonzero. But the rest of the proof works fine if we assume rank $\ell - 1$ here}.
  Suppose that $\str{A} \models \varphi[\elemtuplea]$ (the case of $\str{B} \models \varphi[\elemtupleb]$ is symmetric).
