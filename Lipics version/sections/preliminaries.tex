...
An $\ell$-$\GF$-bisimulation between $\str{A}$ and $\str{B}$ is a sequence $\bisimZ_0, \bisimZ_1, \ldots, \bisimZ_\ell$ of partial isomorphisms\bfside{shouldn't this say ``sets of partial isomorphisms?''} from $\PartIso{\str{A}}{\str{B}}$ mapping live tuples to live tuples such that for all $i < \ell$ we have that $\bisimZ_i$ satisfies~\ref{bisim:forth} and~\ref{bisim:back} conditions for $\bisimZ_{i{+}1}$.
