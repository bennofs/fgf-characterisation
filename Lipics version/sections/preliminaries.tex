...
For pointed structures $(\str{A}, \elemtuplea), (\str{B}, \elemtupleb)$ we use the notation $(\str{A}, \elemtuplea) \equiv_\logicL (\str{B}, \elemtupleb)$ to express that for all $\varphi \in \logicL$ with at most\bfside{``at most'' refering to a variable tuple $\vartuplexfromto{1}{|\elemtuplea|}$ sounds strange to me} $\vartuplexfromto{1}{|\elemtuplea|}$ we have $\str{A} \models \varphi[\elemtuplea]$ if and only if $\str{B} \models \varphi[\elemtupleb]$ (variable $\varx_i$ is substituted with $\elema_i$\bfside{the variable $x_{i}$ is substituted by $b_i$ on the other side of the equality}).
...
\begin{itemize}\itemsep0em
    ...
    \item If $\varphi(\varx)$ has only a single free-variable $\varx$, then $\forall{\varx}\; \varphi$\bfside{using $\varphi(x)$ here would be more consistent} and $\exists{\varx}\; \varphi$ are in $\GF$.
\end{itemize}
The atoms $\alpha$\bfside{here and in the definition: use $\relA$ instead of $\alpha$ for atoms? or not because this any atom, not a concrete relation?} appearing in the 3rd item of the above definition are called \emph{guard}.
...
%
\bfside{This definition mixes tuples and sets: the domain of partial isomorphisms is a set but liveness is a property of tuples}
\begin{description}\itemsep0em
  ...
  \item[\desclabel{(Back)}{bisim:back}] For every $\sigma$-live $\elemtupleb$ in $\str{B}$, there is $\partisog \in \bisimZ'$ with the image $\elemtuplea$ such that $\partisof$ and $\partisog$ agree on their common image.
\end{description}
...
\bfside{$\bisimZ$ should at least be non-empty}% A question: do we want to require that every live tuple from $\str{A}$ has a bisimilar tuple in the other structure? Otto's definition does not require that, and neither does the original definition in Andreka et al 1998}
...
A \emph{system of forward partial maps} between $\sigma$-structures $\str{A}$ and $\str{B}$ is any non-empty subset of \bfside{should this be $i=1$?}$\bigcup_{i=0}^{\infty} (A^i \times B^i)$ satisfying:
\begin{description}\itemsep0em
