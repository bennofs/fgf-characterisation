...
For any finite signature $\sigma$, there is an \possiblelie{exponential}\bbeside{Asked Otto. Waiting for the answer.} function $\homoe \colon \N \to \N$ such that every $\FO[\sigma]$-formula $\varphi$ of quantifier rank $q$ that is $\bisimto_{\GF[\sigma]}$-invariant (in the finite) is also $\bisimto^{\homoe(q)}_{\GF[\sigma]}$-invariant (in the finite) and logically-equivalent (in the finite) to a $\GF[\sigma]$-formula of quantifier rank at most $\homoe(q)$.
\end{theorem}
...
\begin{theorem}
Let $\sigma$ be a finite signature, and let $(\str{A}, \elemtuplea)$ and $(\str{B}, \elemtupleb)$ be a pair of $\ell$-$\FGF[\sigma]$-\emph{bisimilar} $\sigma$-structures.
Then there exist $\FGF$-bisimilar companions $(\str{A}', \elemtuplea')$ and $(\str{B}', \elemtupleb')$ (that are finite if $\str{A}$ and $\str{B}$ are) such that $(\str{A}', \elemtuplea') \bisimto^{\homop(\ell)}_{\GF[\sigma]} (\str{B}', \elemtupleb')$ for some polynomial~function~$\homop$.
\end{theorem}

\bbebox{here we add how this implies the main theorem of the paper}
