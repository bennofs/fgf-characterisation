...
\begin{theorem}\label{thm:main-technical-thm}
  ...
  \bfside{the bisimulation bounds here should be reversed: it should say that if $(\str{A}', \elemtuplea') \bisimto^{\homop(\ell)}_{\FGF[\sigma]} (\str{B}', \elemtupleb')$ then $(\str{A}', \elemtuplea') \bisimto^{\ell}_{\GF[\sigma]} (\str{B}', \elemtupleb')$, since $\homop(\ell) > \ell$}
\end{theorem}
...
\bfside{Is it clear that ``FGF is a fragment of GF'' means that $\bisimto_{\FGF}$-invariance implies $\bisimto_{\GF}$-invariance?}
...
\bfside{here we will show that $\varphi$ is $\bisimto_{\FGF}^{\homop(\ell)}$-invariant (after fixing \cref{thm:main-technical-thm})}
...
From~\cref{thm:main-technical-thm} we infer the existence of $\FGF$-bisimilar companions $(\str{A}', \elemtuplea')$ and $(\str{B}', \elemtupleb')$, that are finite if $\str{A}$ and $\str{B}$ are, for which $(\str{A}', \elemtuplea') \bisimto^{\homop(\ell)}_{\GF[\sigma]} (\str{B}', \elemtupleb')$\bfside{should be $\bisimto^g_{\GF}$ here} holds.
