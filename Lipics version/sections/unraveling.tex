...
We call a structure $\str{A}$ a \emph{forest} if $\relNext^{\str{A}}$ is acyclic, every element has at most one parent and $\str{A}$ contains at least one root.\bbeside{There are still some problems with the definition.}
It is well-known that with a suitable notion of unravelling\bbeside{citation} one can show that every satisfiable modal logic formula has a tree model~\bbeside{tree model undef.}.
The same holds for $\FGF$ as it was shown by Bednarczyk~\cite[Sec 3.3]{Bednarczyk21} with a suitable notion of HAF-unravellings.\bbeside{do we want to define them here?}
Even though the tree unraveling of finite structures can be infinite, model companions used in van Benthem style characterisations for many modal logics~\cite{Otto04}\bbeside{be more specific} can be constructed based on such infinite unravellings.


% Comments:
% The structure $\str{A}$ is a \emph{forest} if every element has at most one parent and there is at least one root.
% Not really, I wouldn't call a cycle+isolated_node a forest :D
% Also notice that according to the current definiton we would have the possibility that there is a copy of Z (integer) + an isolated node. We would like to avoid it, right?
% So maybe after all, the language theoretic definition is better?

Unfortunately, constructing a finite version of the HAF-unraveling is not enough to prove \cref{thm:main-technical-thm}.
Consider the two finite structures shown in \cref{fig:unravel-haf}, which are both already fully HAF-unraveled\bbeside{undef}, but can be distinguished by the $\GF$ sentence $\forall{x,y}. \relE(x,y) \to \exists a. \relP(x,y,z)$.\bbeside{use $\to$ rather than $\implies$.}
This shows that even if the HAF-unraveling is finite, it does not provide $\GF$-bisimilar companions\bbeside{why should they?}\bbeside{maybe this needs to be expanded to make it clearer what you want?}.
\begin{figure}
  \centering
...
\begin{theorem}\label{thm:inf-unraveling-upgrading}
  ...
  Then there are unravelings\bbeside{undef} $\unravel{A}, \elemtuptuplea$ and $\unravel{B}, \elemtuptupleb$ which are both:
  \begin{itemize}
...
\bbebox{It would make sense to define $\FGF$ and $\GF$-games. Then it would be easier to explain bisimulation sequences, as they are just histories of the play.}
Let $\str{A}, \elemtuplea^{(0)}$ be a pointed $\sigma$-structure.
A \emph{bisimulation sequence} of length $\ell$ is a sequence of the form $\elemtuplea^{(0)}(i^{(1)}, j^{(1)})\elemtuplea^{(1)}\cdots(i^{(\ell)}, j^{(\ell)})\elemtuplea^{(\ell)}$, where each $a^{(k)}$ is a live tuple in $\str{A}$ and $i^{(k)}, j^{(k)}$ are indices such that $\elemtupleafromto{i^{(k)}}{j^{(k)}}^{(k-1)} = \elemtupleafromto{1}{j^{(k)}-i^{(k)}+1}^{k}$.\bbeside{we can say something that they indicate the overlap between two consecutive tuples.}
...
Note that the infix selected by the indices $(i^{(k)}, j^{(k)})$ is not required to be maximal.\bbeside{Again, introducing games would make all of this perfectly clear.}
...
\begin{figure}[H]
  \centering
    ...
    \begin{minipage}[t]{0.6\textwidth}
      ...
      The picture on the left shows a structure with the relations: $\relS(1,2,3)$, $\relP(2,3,4)$, $\relT(3,4,5)$, $\relQ(4,5)$\bbeside{you shoudl write $(1,2,3) \in \relS^{\str{A}}$ rather than $\relS(1,2,3)$.}
...
Consider the final tuple\bbeside{maybe use ``last''?} of a bisimulation sequence.
This tuple has a prefix of elements which are shared with the previous tuple, and a suffix of unshared elements.
We now introduce a counter to distinguish these unshared elements.\bbeside{ok}
The \emph{unraveling domain} for $\str{A}$ is a set, defined as follows:
\bfside{This looks really ugly. Any way to write this in a better way?}\bbeside{Rewrite it to get rid of setminus.}
\begin{equation*}
...
The unraveling domain is a forest\bfside{does this require some justification or is it obvious enough?}\bbeside{yes, more explaination = better}, with the binary relation $\relNext \subseteq \unraveldom{A} \times \unraveldom{A}$ defined such that $(s, t) \in \relNext$ iff either\bbeside{if instead of iff, as we define it right now}:
\begin{description}
