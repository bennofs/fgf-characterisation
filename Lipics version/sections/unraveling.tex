...
\begin{theorem}\label{thm:inf-unraveling-upgrading}
  ...
  \end{itemize}
\end{theorem}
Bednarczyk previously described an unraveling for $\FGF$ that yields a tree model, the \emph{HAF-unraveling}~\cite[Sec 3.3]{Bednarczyk21}.
Unfortunately, this unravelling fails the above theorem.
...
For our new \FGF-unravelling, we use the natural interpretation of \FGF-bisimulation as a game.
\bfbox{TODO: Insert description of FGF game here}
...
The \emph{unraveling domain} $\unraveldom{A}$ for $\str{A}$ is a set, with $(\sigma, k) \in \unraveldom{A} \subseteq \seq{A} \times \mathbb{N}$ if:
\begin{itemize}
...
The unraveling domain is a forest\bfside{does this require some justification or is it obvious enough?}\bbeside{yes, more explaination = better}, with the binary relation $\relNext \subseteq \unraveldom{A} \times \unraveldom{A}$ defined such that $(s, t) \in \relNext$ if either:
\begin{description}
