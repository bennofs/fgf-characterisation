...
For $z$-similar elements, we require that the last $z$ moves of both $\seq{e}$ and $\seq{f}$ are equal (or all the moves, if these sequences are shorter than $z$ moves), i.e.\ they both pick the infix at indices $i^{(k)}\ldots{}j^{(k)}$.
This allows us to ``go backwards'' along $\relNext$-edges in the tree, in the following sense:
Let us say that an element $c \in A$ ``undoes'' the last move of an element $a \in A$ if it is a parent of $a$ and $\seq{c}$ is equal $\seq{a}$ but without the final move.
...
Second, $z$ is the bound on the bisimulation equivalence $\elemtuples^{(k)} \bisimto_{\FGF}^{z} \elemtuplet^{(k)}$.
