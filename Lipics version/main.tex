...
\begin{example}
\bbe{TODO: Bennno, please add here an example of a formulae in $\GF$ and an example of a formula that is in $\FO$ but not in $\GF$.}
\end{example}
...
\begin{example}
\bbe{TODO: Bennno, please include examples of formulae that are in $\FGF$ but not in $\GF$.}
\end{example}
...
Moreover, if structures $\str{A}$ and $\str{B}$ are $\omega$-saturated\bbeside{undef}, then the reverse implication hold.
\end{lemma}
\begin{proof}
\bbe{TODO: Benno. Either prove it or find a suitable reference (the second option is preferred.)}
\end{proof}
