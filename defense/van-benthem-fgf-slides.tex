...
\begin{frame}\frametitle{The forward guarded fragment ($\FGF$)}
  ...
  \begin{itemize}
    ...
    \item[\check]
          \alt<2>{
          $\exists{x_{1}x_{2}}\; [\underbrace{\child(x_{1}, x_{2})}_{\text{\textbf{guard}}} \land \underbrace{(\blue(x_{1}) \lor \forall{x_{3}} (\child(x_{2}, x_{3}) \to \green(x_{2})))}_{\text{\textbf{may freely use variables $x_{1}$ and $x_{2}$}}}]$
          }{
          $\exists{x_{1}x_{2}}\; [\child(x_{1}, x_{2}) \land (\blue(x_{1}) \lor \forall{x_{3}} (\underbrace{\child(x_{2}, x_{3})}_{\text{\textbf{guard}}} \to \underbrace{\green(x_{2})}_{\textbf{can use $x_2$, $x_{3}$}}))]$
          }
  \end{itemize}
\end{frame}
...
\begin{frame}<-2>[label=outline]{Outline}%
  ...
  \begin{enumerate}
    ...
    \item<alert@5| check@6> Proof of van Benthem for $\FGF$  \only<4->{
