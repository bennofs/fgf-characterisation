...
\begin{definition}[Context of a path]
  For a bisimulation path $\rho = \bar{s}_{0}(i_{1}, j_{1})\bar{s}_{1}\cdots{}(i_{n}, j_{n})\bar{s}_{n}$, the context is defined as: $\context{\rho} = \text{the last}\ |s_{n}|\ \text{elements of}\ \trace{\rho}$
\end{definition}
...
\begin{definition}[Path-complete tree unraveling]
  ...
  The path-complete unraveling $\mathfrak{A}^{*,c}_{a_{0}}$ is the structure with domain $\mathcal{P} \times \mathbb{N}$ (elements of traces).
  Each relation $R_{i}$ is defined in $\mathfrak{A}^{*,c}_{a_{0}}$ as:
  \begin{equation*}
