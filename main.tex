...
\begin{definition}[Unraveling Domain]
  ...
  \begin{equation*}
    \mathtt{Dom}^{\rightarrow}_{\struct{A}}
      = (\seq{\struct{A}} \times \mathbb{N})
        \setminus
        \left\{ (s(i,j)\bar{a}, k) \mid k \le \dist{i}{j}\ \text{or}\ k > |a| \right\}
  \end{equation*}
  ...
  \begin{enumerate}[(a)]
    ...
    \item $(s, j) \rightarrow (s(i,j)\bar{a}, \dist{i}{j} + 1)$
  \end{enumerate}
\end{definition}
...
\begin{definition}[Infinite Full Tree Unraveling]
  ...
  \begin{equation*}
    \bar{r} \in R_{i}^{\struct{A}^{\rightarrow}} \iff \pi[\bar{r}] \in R_{i}^{\struct{A}} \land \bigwedge_{1 \le i < |\bar{r}|}{r_{i} \rightarrow r_{i+1}} \land |\bar{r}| \le \mathtt{bound}(\bar{r})
  \end{equation*}
  where $\mathtt{bound}(\bar{r})$ is such that $(s,\mathtt{bound}(\bar{r})) = \last{\bar{r}}$ for some bisimulation sequence $s$.
\end{definition}
